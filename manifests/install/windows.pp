# filebeat_legacy::install::windows
#
...
class filebeat_legacy::install::windows {
  ...
  $filename = regsubst($filebeat_legacy::real_download_url, '^https?.*\/([^\/]+)\.[^.].*', '\1')
  $foldername = 'Filebeat'
  $zip_file = join([$filebeat_legacy::tmp_dir, "${filename}.zip"], '/')
  $install_folder = join([$filebeat_legacy::install_dir, $foldername], '/')
  ...
  if ! defined(File[$filebeat_legacy::install_dir]) {
    file { $filebeat_legacy::install_dir:
      ensure => directory,
    }
  }
  ...
  archive { $zip_file:
    source       => $filebeat_legacy::real_download_url,
    cleanup      => false,
    creates      => $version_file,
    proxy_server => $filebeat_legacy::proxy_address,
  }

  exec { "unzip ${filename}":
    command => "\$sh=New-Object -COM Shell.Application;\$sh.namespace((Convert-Path '${filebeat_legacy::install_dir}')).Copyhere(\$sh.namespace((Convert-Path '${zip_file}')).items(), 16)", # lint:ignore:140chars
    creates => $version_file,
    require => [
      File[$filebeat_legacy::install_dir],
      Archive[$zip_file],
  ...
  exec { "rename ${filename}":
    command => "Remove-Item '${install_folder}' -Recurse -Force -ErrorAction SilentlyContinue;Rename-Item '${filebeat_legacy::install_dir}/${filename}' '${install_folder}'", # lint:ignore:140chars
    creates => $version_file,
